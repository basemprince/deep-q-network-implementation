...
def get_critic(nx):
    ...
    outputs = layers.Dense(NU*JOINT_COUNT)(state_out4)
...
def save_model():
    ...
    name = "Q_weights_backup/tr/Q_weights_" + eps_num + ".h5"
    Q.save_weights(name)
#    simulate_sp(eps_num)
...
if __name__=='__main__':
    ...
    directory = glob.glob('Q_weights_backup/tr/*')
...
#    
    ...
    #                    env.render()
                        print(x , cost)
                    replay_buffer.append([x, cost, x_next,reached])
                    ...
                    if len(replay_buffer) >= MIN_BUFFER_SIZE and steps % SAMPLING_STEPS == 0:
                        x_batch, cost_batch, x_next_batch, reached_batch = zip(*sample(replay_buffer, BATCH_SIZE))
                        x_batch = tf.convert_to_tensor(np.asarray(x_batch).squeeze())
                        x_next_batch = tf.convert_to_tensor(np.asarray(x_next_batch).squeeze())
                        cost_batch1 = np2tf(np.asarray(cost_batch))
                        buffer_size = len(x_batch)
                        with tf.GradientTape() as tape:
                            # Compute Q target
                            target_values_per_joint_input = Q_target(x_next_batch, training=True).reshape(buffer_size,-1,JOINT_COUNT)
                            target_values_per_joint = np.min(target_values_per_joint_input, axis=1)
                            target_values_per_input = tf.math.reduce_sum(target_values_per_joint,axis=1)
                            ...
                            Q_values_per_joint_input = Q(x_batch, training=True).reshape(buffer_size,-1,JOINT_COUNT)
                            Q_values_per_joint = np.min(Q_values_per_joint_input, axis=1)
                            Q_values_per_input = tf.math.reduce_sum(Q_values_per_joint,axis=1)
                            # Critic's loss function. tf.math.reduce_mean() computes the mean of elements across dimensions of a tensor
                            Q_loss = tf.math.reduce_mean(tf.math.square(y - Q_values_per_input)) 
...
#                    name = "Q_weights_backup/tr/Q_weights_" + str(episode).zfill(5) + ".h5"
#                    Q.save_weights(name)
                    best_ctg = avg_ctg
            ...
            name = "Q_weights_backup/tr/Q_weights_final.h5"
            Q.save_weights(name)
